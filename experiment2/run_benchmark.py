...
def resolve_video_frame_paths(samples: Iterable[dict], root: str | Path | None) -> dict[str, str]:
    """Resolve image paths for video-frame samples based on a root directory."""
    ...
    for sample in samples:
        image_id = str(sample.get("imageId", "")).strip()
        metadata = sample.get("metadata") or {}
        direct_path = sample.get("imagePath") or metadata.get("image_path")
        if direct_path:
            candidate = Path(direct_path).expanduser()
            if candidate.is_file():
                resolved[image_id] = str(candidate)
                continue
        video_id = str(metadata.get("video_id", "")).strip()
        candidates: list[Path] = []
        if video_id:
            candidates.append(base_root / video_id / image_id)
        candidates.append(base_root / image_id)
...
def build_prompts(
    ...
    for sample in dataset:
        ...
        if not image_path:
            inline_path = sample.get("imagePath") or (sample.get("metadata") or {}).get("image_path")
            if inline_path:
                image_path = str(Path(inline_path).expanduser())
        metadata_extra = {}
        metadata_extra.update(sample.get("metadata") or {})
        metadata_extra.update(sample.get("_metadata", {}) or {})
        if image_path:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="gqa",
        help="Dataset to evaluate (GQA, LLaVA-Instruct-150K, the synthetic cache demo, or custom video frames).",
    )
    parser.add_argument(
        "--gqa-image-dir",
    ...
    parser.add_argument(
        "--video-frames-data",
        default="dataset_custom/video_frames_labels.json",
        help="Path to video frames dataset JSON file (used when --dataset video_frames).",
    )
    parser.add_argument(
        "--video-frames-root",
        default=None,
        help="Root directory containing extracted frame images (defaults to directory of the labels file).",
    )
    parser.add_argument(
        "--trust-remote-code",
        action="store_true",
    ...
    if getattr(args, "use_hf", False) and getattr(args, "cache_backend", "semantic-kv") == "semantic-kv":
        print("[info] --use-hf enabled; switching cache backend to model-router (no vLLM KV injection).")
        args.cache_backend = "model-router"
    return args
...
def main() -> None:
    ...
    if args.dataset == "gqa":
        ...
        data_path = Path(args.video_frames_data).expanduser()
        if not data_path.exists():
            raise FileNotFoundError(f"Video frames metadata not found at {data_path}")
        base_samples = load_video_frames_dataset(str(data_path), limit, shuffle_seed)
        dataset_label = f"video-frames ({data_path.name})"
        args.dataset_config = data_path.name
        root_hint = args.video_frames_root or os.environ.get("GQA_IMAGE_ROOT") or str(data_path.parent)
        image_path_map = resolve_video_frame_paths(base_samples, root_hint)
    else:
